...
WORKDIR /app
COPY --from=builder /graphiti/server/pyproject.toml /graphiti/server/README.md /graphiti/server/uv.lock ./
COPY --from=builder /graphiti/server/graph_service /graphiti/graph_service
