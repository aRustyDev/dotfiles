...
RUN gh repo clone getzep/graphiti /graphiti
...
# TODO: symlink main.py to /src/graphiti
COPY --from=builder /graphiti/mcp_server/main.py ./
COPY --from=builder /graphiti/mcp_server/src/ /src/graphiti
COPY --from=builder /graphiti/mcp_server/config/ /etc/
...
# Run the MCP server
CMD ["uv", "run", "--no-sync", "graphiti_mcp_server"]
# CMD ["uv", "run", "--config", "/etc/graphiti.yaml", "graphiti_mcp_server.py"]
